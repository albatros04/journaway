...
export function CabExperience({ initialTrip = { pickup: "", drop: "", date: "" } }: { initialTrip?: TripDetails }) {
  ...
  return <main className="cab-experience">
    ...
    {showReview && <section className="cab-review-section" id="cab-review" aria-live="polite"><div className="container"><div className="cab-trip-summary"><div><p className="eyebrow">Your travel request</p><h2>{trip.pickup} <span aria-hidden="true">→</span> {trip.drop}</h2><p>{readableDate(trip.date)}</p></div><button className="edit-search" type="button" onClick={() => document.getElementById("cab-search")?.scrollIntoView({ behavior: "smooth", block: "center" })}>Edit search</button></div><div className="cab-enquiry-next"><div><p className="eyebrow">Next step</p><h3>Let’s shape the right ride around your route.</h3><p>Share your contact details and the JournAway team will confirm vehicle choices and fares for your route.</p></div><EnquiryAccountGate returnTo="/vehicle-rental">{customer => <form className="form-grid" onSubmit={submitEnquiry}><input className="form-honeypot" name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" /><label>Your name<input required name="name" autoComplete="name" defaultValue={customer.displayName} readOnly /></label><label>Phone number<input required name="phone" type="tel" autoComplete="tel" /></label><label className="form-full">Email address<input required name="email" type="email" autoComplete="email" defaultValue={customer.email} readOnly /></label><button className="button button-primary" disabled={sendingEnquiry}>{sendingEnquiry ? "Sending…" : "Send cab enquiry"}</button></form>}</EnquiryAccountGate>{enquiryMessage && <p className="form-notice" role="status">{enquiryMessage}</p>}</div></div></section>}
