...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!await getOperationsDb().select({ id: driverProfiles.id }).from(driverProfiles).where(eq(driverProfiles.id, driverProfileId)).get()) return Response.json({ error: "Driver profile not found." }, { status: 404 });
